...
All programming requires is a creative mind and the ability to organize your thoughts. The greatest limitation in writing software is our ability to understand the systems that we are creating.
...
A module has two parts: an interface and an implementation.
The interface describes what the module does but not how it does it.
A developer should not understand the implementations of modules other than the one he or she is working in.

An interface contains two kinds of information: formal and informal.
The formal parts of an interface are specified explicitly in the code.
Informal information can only be described using comments (e.g. behaviour of a function).

Abstraction can help you to make it easier for us to think about and manipulate complex things.
...
Modules that provide powerful functionality yet have simple interfaces are called **deep**.
A **shallow** module is on whose interface is complicated relative to the functionality it provides.

One of the most important techniques for achieving deep modules is **information hiding**.

The opposite is information leakage. One common case is temporal decomposition:
The structure of a system corresponds to the time order in which operations will occur (execution order is reflected in the code structure).
...
Eliminating special cases can also make code more efficient.
...
The overall idea behind comments is to capture information that was in the mind of the designer but could not be represented in the code.
One of the purposes of comments is to make it unnecessary to read the code.
Developers end up effectively retyping the documentation for a method every time they invoke it.
...
Comment categories:
- Interface Comments
- Data Structure Comments (Comment next to the declaration of a field)
- Implementation Comments (Comment inside the code of a method; are often unnecessary)
- Cross-Module Comments (Comment describing dependencies that cross module boundaries)
...
The main goal of implementation comments is to help readers understand what the code is doing (not how).
...
If a variable or method name is broad enough to refer to many things, then it does not convey much information to the developer and the underlying entity is more likely to be misused.
If it is hard to find a name, then it is a hint that the underlying object may not have a clean design.
Every word is a name should provide useful information.
It is not recommended that the type information or class information is included in the name.

Delayed comments are bad comments. For a new class, write the class interface comments first.

If you find it difficult to write such a comments, that is a hint that there may be a problem with the design of the thing you are describing.

Early Comments are fun comments: These comments record and test the quality of the design decisions.
Writing comments first will mean that the abstraction will be more stable before you start writing code.
This will probably save time during coding.
...
When developers go into exiting code, they do not think strategically.
They think "What is the smalled possible change I can make that does what I want?".
As a result, the system design gets just a bit worse, and the problems accumulate with each step in teh system's evolution.
Ideally, when you have finished with each change, the system will have the structure it would have had if you had to designed it from the start with that change in mind.
Even if your change does not require refactoring, you should still be on the lookout for design imperfections that you can fix while you are in the code.
If you are not making the design better, you are probably making it worse.

When you change existing code, there is a good chance that the changes will invalidate some existing comments.
...
When writing a commit message, ask yourself whether developers will need to use the information in the future.
If documentation is duplicated, it is more difficult to find and update all the relevant copies.
...
The best way to determine the obviousness of the code is through code reviews.

Generic Containers are not helpful to understand: Use a new class.
